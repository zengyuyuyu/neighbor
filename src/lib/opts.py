...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='coco',
                             help='coco | kitti | coco_hp | pascal | coco_cp')
    ...
    self.parser.add_argument('--last_next', default=True, action='store_true')
    # exdet
  ...
  def parse(self, args=''):
    ...
    opt.save_dir = os.path.join(opt.save_dir, 'model')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.last_next:
        opt.heads.update({'ln_cls': 2, 'ln_wh': 4})
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 256], 'num_classes': 5,
                'mean': [0.379, 0.379, 0.379], 'std': [0.229, 0.229, 0.229],
                'dataset': 'coco_cp'},
