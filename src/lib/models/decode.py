...
def _topk_from_current_adj(current, last, last_neg, next, next_neg, topk_ind):
    last_valid = torch.masked_select(last, ~last_neg).view(1, -1)
    next_valid = torch.masked_select(next, ~next_neg).view(1, -1)
    return torch.cat((current, last_valid, next_valid), dim=1).gather(1, topk_ind)
...
def _topk_self_adj(scores, ln_cls, ln_ct, K=40):
    batch, cat, height, width = scores.size()
    ...
    ln_cls = _transpose_and_gather_feat(ln_cls, topk_inds).sigmoid()
    ln_ct = _transpose_and_gather_feat(ln_ct, topk_inds)
    last_scores = topk_scores * ln_cls[:, :, 0]
    next_scores = topk_scores * ln_cls[:, :, 1]
    ln_ct_wh = ln_ct.int().float()
    last_xs, last_ys, next_xs, next_ys = ln_ct_wh[:, :, 0], ln_ct_wh[:, :, 1], ln_ct_wh[:, :, 2], ln_ct_wh[:, :, 3]
    last_inds = (last_ys * width + last_xs).to(torch.int64)
    next_inds = (next_ys * width + next_xs).to(torch.int64)
    last_clses = topk_clses + 1
    next_clses = topk_clses - 1

    last_neg1 = ln_cls[:, :, 0] < 0.75
    last_neg2 = last_clses > 4
    last_neg = last_neg1.__or__(last_neg2)
    next_neg1 = ln_cls[:, :, 1] < 0.75
    next_neg2 = next_clses < 0
    next_neg = next_neg1.__or__(next_neg2)

    last_ct_neg_xs = (last_xs < 0) & (last_xs > width)
    last_ct_neg_ys = (last_ys < 0) & (last_ys > height)
    next_ct_neg_xs = (next_xs < 0) & (next_xs > width)
    next_ct_neg_ys = (next_ys < 0) & (next_ys > height)
    last_ct_neg = last_ct_neg_xs.__or__(last_ct_neg_ys)
    next_ct_neg = next_ct_neg_xs.__or__(next_ct_neg_ys)
    last_neg = last_neg.__or__(last_ct_neg)
    next_neg = next_neg.__or__(next_ct_neg)
    last_scores = torch.masked_select(last_scores, ~last_neg).view(1, -1)
    next_scores = torch.masked_select(next_scores, ~next_neg).view(1, -1)

    final_scores, topk_3k_indices = torch.topk(torch.cat((topk_scores, last_scores, next_scores), dim=1), k=K)

    final_inds = _topk_from_current_adj(topk_inds, last_inds, last_neg, next_inds, next_neg, topk_3k_indices)
    final_clses = _topk_from_current_adj(topk_clses, last_clses, last_neg, next_clses, next_neg, topk_3k_indices)
    final_ys = _topk_from_current_adj(topk_ys, last_ys, last_neg, next_ys, next_neg, topk_3k_indices)
    final_xs = _topk_from_current_adj(topk_xs, last_xs, last_neg, next_xs, next_neg, topk_3k_indices)

    return final_scores, final_inds, final_clses, final_ys, final_xs
...
def ctdet_decode(heat, wh, reg=None, ln_cls=None, ln_ct=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    heat = _nms(heat)

    if ln_cls is None and ln_ct is None:
        scores, inds, clses, ys, xs = _topk(heat, K=K)
    else:
        scores, inds, clses, ys, xs = _topk_self_adj(heat, ln_cls, ln_ct, K=K)
    if reg is not None:
      reg = _transpose_and_gather_feat(reg, inds)
      reg = reg.view(batch, K, 2)
      xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
